...
./gradlew test
./gradlew :feature:characters:test                       # single module
./gradlew :feature:characters:test --tests "*CharactersViewModelTest*"   # single test
...
- `rickandmorty.android.feature` — the above + Compose, Koin, lifecycle (for UI features).
...
error strings + fonts; `:feature:characters` owns everything else it renders.
...
(e.g. `CharacterFixtures.kt`, `FakeRepositories.kt` in `:feature:characters`).
